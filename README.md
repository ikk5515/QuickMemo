...
Google 로그인은 Functions 없이 Firebase Auth 클라이언트 SDK만으로 동작합니다. 단, QuickMemo의 노트 개인키는 사용자 비밀번호에서 파생한 키로 암호화되어 있으므로 Google 로그인 후에도 `/app`에서 노트 복호화를 위해 기존 QuickMemo 비밀번호를 한 번 입력해야 합니다.

1. Firebase Console > Build > Authentication > Sign-in method에서 Google 제공업체를 활성화하고 저장합니다.
2. Authentication > Settings > Authorized domains에 로컬/배포 도메인이 있는지 확인합니다.
   - 로컬: `localhost`, `127.0.0.1`
   - Vercel: `your-project.vercel.app`, 연결한 커스텀 도메인
3. 사용자는 먼저 원형 사용자 버튼 + 비밀번호로 기존 QuickMemo 계정에 로그인합니다.
4. 상단의 `Google 연결` 버튼으로 본인의 Google 계정을 현재 QuickMemo 계정에 연결합니다.
5. 이후 로그인 화면의 `Google로 로그인` 버튼으로 접속할 수 있습니다.

Google 계정은 기존 QuickMemo 사용자에 연결된 경우에만 허용됩니다. 연결되지 않은 Google 계정으로 로그인하면 앱이 별도 프로필 없는 Firebase Auth 계정을 즉시 정리하고 로그인을 거부합니다.
