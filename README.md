...
- `@dnd-kit` 기반 일정 매트릭스 Drag & Drop
- Intl dangi calendar 기반 한국 공휴일 계산과 보완 공휴일 매핑

## 주요 기능

- `/setup`: 첫 관리자 계정을 생성합니다.
- `/login`: 이름이 표시된 사용자 목록에서 계정을 선택하고 로그인합니다.
...
- 주요 메뉴는 **메모 · 위키 · 일정 · 자료실**입니다. 사용자 설정과 관리 기능은 작업공간 메뉴에서 엽니다.
- 메모 왼쪽에서는 파일, 검색, 북마크를 바로 찾을 수 있습니다. 태그, 연결 보기, 가져오기·내보내기, 휴지통은 **더 보기**에 모았습니다. 날짜별 메모 달력은 필요할 때 펼칩니다.
- **Base · Kanban · Canvas 생성 및 전용 편집 화면을 제거했습니다.** 이미 저장된 `.base`와 `.canvas` 파일은 읽기 전용으로 원문을 확인하고 내려받을 수 있습니다. 기존 Kanban은 Markdown 원문으로 열립니다. 암호화 데이터와 ZIP 가져오기·내보내기 호환성은 유지합니다.
- 링크·검색·백링크, 암호화 변경 이력, 첨부파일, 안전한 Markdown 미리보기는 계속 사용할 수 있습니다. 템플릿·드로잉 같은 보조 도구는 명령 팔레트에서 찾습니다.
- 화면 너비에 맞춰 패널과 메뉴가 배치되며 모바일에서는 메모 본문 공간을 우선합니다. 밝은 테마와 어두운 테마, 키보드 탐색을 지원합니다.
- 사용자 JavaScript와 외부 플러그인 바이너리를 실행하지 않습니다. 별도 API 키·종량제 AI·새 데이터베이스를 추가하지 않습니다.

Firestore 동기화는 백업이 아닙니다. 변경 이력과 ZIP 내보내기를 별도 복구 수단으로 사용할 수 있습니다. 충돌한 초안을 자동으로 덮어쓰지 않으며, 저장 상태와 오류를 구분해 표시합니다.

### 나의 위키와 반복 로딩 최적화

- **위키**는 `/wiki`에서 내 Markdown·기존 HTML 메모를 읽고 검색하는 전용 화면입니다. 폴더 탐색, 목차, 내부 링크, 백링크와 로컬 그래프를 제공합니다. 공개 게시가 아니며 로그인·활성 계정·메모 권한·암호화 잠금 해제가 필요합니다. 위키 열람은 문서 수정이나 폴더 자동 복구를 실행하지 않습니다.
- 메모의 **더 보기 → 위키 새 탭에서 읽기**는 저장을 확인한 뒤 별도 브라우저 탭을 엽니다. 다른 탭으로 암호화 키를 전달하지 않으므로 새 탭에서는 다시 잠금을 해제합니다. 같은 탭에서 메모와 위키를 이동할 때는 현재 잠금 해제 세션을 재사용합니다.
- 세션 캐시는 최대 5,000개 항목·추정 32MiB 범위에서 정확한 암호문과 권한이 같은 키·필드만 재사용합니다. 본문만 바뀌면 본문만 복호화하며, 동일한 서버 메타데이터 알림은 편집 화면을 다시 로딩하지 않습니다. 동시 암호 연산은 4개로 제한합니다. 잠금·로그아웃·계정 또는 권한 변경 시 캐시와 진행 작업을 무효화하며 브라우저 저장소에 키·평문을 기록하지 않습니다.
- 새 항목의 이름 검증용 HMAC과 키 생성은 병렬로 실행하고, 저장에는 현재 세션의 검증된 노트 키를 재사용합니다. 새 메모는 서버 생성 확정과 구독 반영을 확인한 뒤 편집할 수 있습니다. 최초 잠금 해제와 네트워크 왕복까지 없어지는 것은 아닙니다.
- 그래프는 설정만 바뀔 때 기존 노드 좌표를 유지하고, 연결 강조·테마 대비·키보드 이동과 힘 조절을 개선했습니다. 그래프 데이터의 반복 계산은 데이터 변경 시 폐기하는 제한된 캐시로 줄입니다. 검증과 측정 방법은 [성능·위키 변경 기록](docs/vault-session-wiki.md)을 참고하세요.
