...
test("explicitly publishes one image without granting its source folder or sibling image", async ({ page, request, browser }, testInfo) => {
  ...
  await expect(create).toBeEnabled({ timeout: 40_000 }); await create.click();
  ...
  await dialog.getByRole("checkbox", { name: `${title}.md`, exact: true }).check();
  ...
  await expect(preview.getByText("메모 1개 · 이미지 1개 · 폴더 0개", { exact: true })).toBeVisible();
  ...
  try {
    ...
    expect(manifest.entries.every((entry) => entry.folderId === null)).toBe(true);
